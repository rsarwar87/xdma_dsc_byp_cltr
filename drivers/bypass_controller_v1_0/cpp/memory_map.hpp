...
constexpr uint32_t get_mmap_id(const MemID id) {
...
template <MemID id, uintptr_t phys_addr = mem::get_base_addr(id),
...
class Memory {
 public:
  ...
  Memory() : mapped_base(nullptr), base_address(0), is_opened(false) {}

  ~Memory() {
    if (is_opened) munmap(mapped_base, size);
  }

  void open( int &fd) {
    mapped_base = mmap(nullptr, size, protection, MAP_SHARED, fd,
                       get_mmap_offset(phys_addr, size));
    ...
    is_opened = true;
    base_address =
        reinterpret_cast<uintptr_t>(mapped_base) + (phys_addr & (size - 1));
  }
  ...
  auto get_params() {
    return std::make_tuple(base_address, is_opened, phys_addr, size,
                           protection);
  }
  ...
  void write_array(const std::array<T, N> &arr) {
    static_assert(offset + sizeof(T) * (N - 1) < mem::get_range(id),
                  "Invalid offset");
    static_assert(mem::is_writable(id), "Not writable");
  ...
  void write_mask(uint32_t value) {
    ...
    *(volatile uintptr_t *)addr =
        (*((volatile uintptr_t *)addr) & ~mask) | (value & mask);
  }
  ...
  T *get_ptr(uint32_t block_idx = 0) {
    ...
    return reinterpret_cast<T *>(base_address + block_size * block_idx +
                                 offset);
  }
  ...
  std::array<T, N> &read_array(uint32_t block_idx = 0) {
    static_assert(offset + sizeof(T) * (N - 1) <
                      (mem::get_range(id) * mem::get_n_blocks(id)),
                  "Invalid offset");
    static_assert(mem::is_readable(id), "Not readable");
  ...
  void clear_bit() {
    ...
    uintptr_t addr = base_address + offset;
    *(volatile uintptr_t *)addr =
        *((volatile uintptr_t *)addr) & ~(1U << index);
  }
