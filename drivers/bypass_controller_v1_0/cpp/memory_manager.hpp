...
class MemoryManagerImpl<N, std::index_sequence<ids...>> {
 public:
  MemoryManagerImpl() : fd(-1), failed_maps(0) {}
 ...
 private:
  ...
  template <MemID id>
  void open_memory_map();

  template <MemID cnt>
  std::enable_if_t<cnt == 0, void> open_maps() {}
...
int MemoryManagerImpl<N, std::index_sequence<ids...>>::open() {
  if (mem::get_mmap_id(N) == 0)
    fd = ::open("/dev/mem", O_RDWR | O_SYNC);
  else if (mem::get_mmap_id(N) == 1)
    fd = ::open("/dev/xdma0_user", O_RDWR | O_SYNC);
  else if (mem::get_mmap_id(N) == 2)
    fd = ::open("/dev/xdma0_control", O_RDWR | O_SYNC);
